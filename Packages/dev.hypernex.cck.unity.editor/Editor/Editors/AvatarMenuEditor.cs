...
namespace Hypernex.CCK.Unity.Editor.Editors
{
    ...
    public class AvatarMenuEditor : UnityEditor.Editor
    {
        ...
        private (int, string) ValidParameters()
        {
            if (AvatarMenu.Parameters == null) return (3, "Please set your Avatar's Parameters!");
            if (AvatarMenu.Parameters.Parameters.Length <= 0) return (2, "No parameters are present!");
            return (0, String.Empty);
        }

        private GUIContent[] GetOptionsFromParameters(AvatarParameters parameters) =>
            new[] {new GUIContent("None")}
                .Union(parameters == null ? Array.Empty<GUIContent>() : parameters.Parameters.Select(x => new GUIContent($"{x.ParameterName} ({x.ParameterType})")))
                .ToArray();
        ...
        {
            (int, string) validParams = ValidParameters();
            if(validParams.Item1 > 0)
                EditorUtils.DrawSpecialHelpBox((MessageType) validParams.Item1, validParams.Item2);
